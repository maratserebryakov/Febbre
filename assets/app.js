  ...
  async function bootSongPage() {
    ...
    btnEnd.addEventListener("click", () => {
      ...
      if (it.start != null && Number(it.end) <= Number(it.start)) {
        toast("End должен быть больше Start");
        it.end = null;
      }
      save();
      renderLines();
    ...
    function renderLines() {
      ...
      state.items.forEach((it, idx) => {
        ...
        userInput.addEventListener("input", () => {
          it.phonetic_user = userInput.value;
          save();
        });
    ...
    btnExport.addEventListener("click", async () => {
      ...
      try {
        ...
        toast("Экспортировано", "JSON в буфере обмена");
      } catch {
        toast("Экспортировано", "Скопируйте из поля вручную");
      }
    });
